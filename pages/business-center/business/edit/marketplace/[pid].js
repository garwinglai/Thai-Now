...
import {
  publishDraftBusinessMarketPost,
  updateMarketBusinessPost,
  saveMarketBusinessDraft,
  updateMarketBusinessDraft,
...
function EditMarketplace({ pid }) {
  ...
  useEffect(() => {
    ...
    const fetchHousingPost = async () => {
      const postRef = doc(db, "users", uid, "biz", bizId, "marketPosts", pid);
  ...
  const publishPost = async () => {
    ...
    if (isDraft) {
      const { success, error, postId } = await publishDraftBusinessMarketPost(
        marketPostData,
        uid,
        bizId
      );
      ...
      const { success, error, postId } = await updateMarketBusinessPost(
        marketPostData,
        uid,
        bizId
      );
  ...
  const handleSaveAndExit = async () => {
    ...
    if (isDraft) {
      const { success, error } = await updateMarketBusinessDraft(
        housingPostData,
        uid,
        bizId
      );
