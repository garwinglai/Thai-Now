...
export default async function handler(req, res) {
  ...
  const { uid, newPostId, oldPostId, fileName, bizId } = body;
  if (method === "POST") {
    const storage = new Storage();

    const destFileName = `users/${uid}/biz/${bizId}/marketplace/${newPostId}/${fileName}`;
    const srcFileName = `users/${uid}/biz/${bizId}/drafts/${oldPostId}/${fileName}`;
